...
| `execute` | `(args, exec) => Promise` | 执行函数 |
...
↓ tools/pre-execute — 审批和守卫（allow / deny / ask）

↓ tools/execute — 围绕式中间件（超时、重试、指标）

↓ 工具体执行 — execute(args, exec)

↓ tools/post-execute — 结果后处理（accept / replace / block）

↓ output.render() — 规范化投影为模型可见内容

↓ session.append('tool/result') — 记入会话日志

#### pre-execute 阶段

审批和守卫。监听器可以允许（next()）、拒绝或请求用户审批。这是安全策略的第一道门。
...
工具调用的并行调度在 executeToolCalls() 中实现，采用有界滚动池模式：
...
每个工具必须声明 output 字段，包含 JSON Schema 验证和纯投影函数：
...
- 运行 `system-prompt/assemble` waterfall
...
LLM 适配器通过 ctx.llm 注册。每个适配器声明支持的 provider/model 路由、上下文窗口大小、默认值和重试策略。
...
- 从日志恢复持久化头（`session.requestHeader()`）

- 移除适配器派生的默认值

- 通过 `agent/request` waterfall 让插件替换

- 通过 `ctx.llm.prepareCall()` 解析适配器

- 比较变化，按需记录 `request/header` 事件
