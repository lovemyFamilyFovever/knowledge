...
DeepSeek Harness 核心架构分析 — 第六部分：LLM 适配器层
...
键
llm
核心 LLM 服务：适配器注册表、流式调用 API、类型定义、错误处理、块组装器
ctx.llm
llm-deepseek
DeepSeek 适配器实现（OpenAI 兼容 SSE 协议）
—
llm-pi-ai
Pi AI 适配器实现
—
...
17.2 核心抽象
LLM 层的核心抽象是
provider 路由 + 适配器注册 + 流式 waterfall
：
Agent Loop
...
职责
registerAdapter()
注册适配器到 provider 路由，返回可原子替换的句柄
registerConfigurableProviders()
声明可通过配置激活的 provider 目录
...
：
prepareCall()
方法将模型元数据和流式调度绑定到同一个适配器代次，防止设置变更在准备和调度之间组合不同代次的能力。
18.3 BlockAssembler
...
喂入一个 chunk，增量更新组装状态
blocks()
组装所有已见的块（max-token 截断时丢弃不安全的 tool-call）
interruptedBlocks()
中断流的安全前缀（只保留有内容的 text/reasoning）
usage
来自
usage
chunk 的 token 计量
...
19.3 适配器边界
adapterStream()
是最终的适配器边界。关键设计：适配器选择、调度和迭代器构建中的异常都转换为流协议的终端 finish chunk，而不是抛出。中间件和下游消费者的异常保持抛出。
20. 适配器注册与路由
20.1 注册流程
验证 provider 名称非空且唯一
调用
...
目录是适配器插件可以通过配置激活的 provider 列表。配置表面将此目录与
listProviders()
合并，提供每个可配置 provider 的在线/离线状态。
21. 错误处理体系
21.1 错误层次
Error
...
通过正则表达式匹配多种 provider 的上下文溢出措辞，包括结构化码、最大限制、请求过大和超过模型上下文等模式。
22. 重试策略
22.1 策略配置
模式
说明
...
DeepSeek 适配器是
transport-only
：连接事实通过 thunk 解析，bearer token 通过 per-request 解析器获取。注册插件拥有验证、分层和凭据策略。
23.2 推理努力级别
...
（
ctx.tokenMeter
）是全应用范围的 token 计量服务，具有重放感知和增量同步能力。
24.2 计量模型
Token 使用量是
不相交
的：
