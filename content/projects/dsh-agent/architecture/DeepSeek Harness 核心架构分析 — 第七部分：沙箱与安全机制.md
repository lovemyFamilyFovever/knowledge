...
策略是 per-call 的：两个消费者可以在同一时刻以不同策略限制执行。
...
| 平台 | 实现 |
| --- | --- |
| Linux | Landlock + bwrap（Bubblewrap）链 |
| macOS | sandbox-exec（Seatbelt） |
| Windows | ACL 受限令牌（per-session/per-workspace SID） |

## 30. 安全设计原则

- **失败关闭**：无后端可用时拒绝运行，不静默通过

- **Per-call 策略**：策略按调用携带，不固定在提供者上

- **升级需审批**：更宽的模式需要用户明确批准

- **拒绝签名隔离**：每个后端有自己的拒绝方言

- **运行器失败 vs 拒绝**：运行器失败 ≠ 拒绝（限制工作并阻止了命令）
