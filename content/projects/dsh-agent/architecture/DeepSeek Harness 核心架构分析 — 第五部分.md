...
第五部分：Subagent 委派机制与 Compaction（压缩）机制
...
- 父 Agent 调用 `ctx.agents.create(options)`

- `options.meta` 携带血统信息：`parentSession`、`delegationDepth`、`origin: 'subagent'`、`agentPreset`

- `options.seed` 可以传递父 Session 的历史前缀（fork 场景）

- 工厂创建 Session + Agent，通过 `AgentSetup` 回调组合子 Agent 的世界
...
子 Agent 可以使用与父 Agent 不同的模型。installModelSelection() 函数将可变的模型选择耦合到 Agent 作用域：
...
foldConsumedWork() 函数追踪 Agent 日志中已消费的工作，解决了轮次词汇表无法区分"在首个步骤前停止的轮次"和"平衡的空操作轮次"的问题。

## 14. Compaction（压缩）机制

### 14.1 服务定义

CompactionEngine 是一个抽象服务（ctx.compaction），由具体的后端实现。它负责决定何时压缩、选择压缩范围、生成摘要和替换历史范围。
...
- `compaction/start` → 记录锁定标记

- 选择压缩范围（必须平衡：assistant tool calls 与 results 配对）

- 调用 LLM 生成摘要

- `compaction/summary` → 记录摘要和元数据

- `user/message`（surfaceOp: replace）→ 替换选定的表面范围
...
Surface 是 Session 日志之上的有序视图，只包含模型可见的消息事件（user/message、assistant/message、tool/result）。
...
- **错误遏制**：每层边界都有独立的错误遏制机制

核心格言：一切模型可见的上下文都是已记录的事实，一切行为都是可替换的插件，一切状态都是可投影的事件流。

DeepSeek Harness 核心架构分析 — 第五部分 | 生成日期：2026-08-29
