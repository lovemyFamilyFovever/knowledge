...
> 📌 **导航**：本文是 HTTP 的**综合参考**（方法/状态码/头部/缓存/Cookie/HTTP2/3）。其中安全与加密（HTTPS/TLS）详见 [[HTTPS 与 TLS]]，长连接双向通信详见 [[WebSocket]]，缓存与分发详见 [[内容分发网络]]，传输层详见 [[TCP深入]]。
...
[[HTTPS 与 TLS]]、[[TCP深入]]、[[WebSocket]]、[[内容分发网络]]、[[网络基础]]、[[应用层协议]]

## 参考资料

建议人工核验：可参考 RFC 9110（HTTP 语义）、RFC 9111（缓存）、RFC 9112（HTTP/1.1）、RFC 9113（HTTP/2）、RFC 9114（HTTP/3）（均 2022 年发布），以及 MDN Web Docs 的 HTTP 专区。
