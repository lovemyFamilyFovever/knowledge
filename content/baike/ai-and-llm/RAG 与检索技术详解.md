...
> 🎯 RAG = 先检索再生成，专治知识过时、幻觉、私有数据三大痛点。链路四层：查询改写（含 HyDE）→ 检索（BM25 关键词 + 向量语义，混合用 RRF 融合）→ Cross-Encoder 重排 → 把片段拼进 Prompt 让 LLM 带引用作答。地基是文档分块（递归分块、设 overlap），质量用 Recall@K / MRR / NDCG 评估；Embedding 与向量库是两大选型关键。优点是知识可更新、可溯源，代价是上限受检索质量约束、链路增延迟。
